...
class IsoSurfaceRenderObject(RenderObject):
    ...
    def create_cut_trigs(self, count=None):
        ...
        mesh_pts = self.mesh.MapToAllElements(
        ...
        self.device.queue.writeBuffer(vertex_buffer, 0, vertices.tobytes())
        ...
        self.device.queue.writeBuffer(
            only_count,
            0,
            np.array([1 if count is None else 0], dtype=np.uint32).tobytes(),
            0,
            4,
        )
        self.device.queue.writeBuffer(
        ...
        bindings = [
            ...
            *self.gpu.u_mesh.get_bindings(),
            ...
            BufferBinding(83, function_value_buffer, visibility=ShaderStage.ALL),
            BufferBinding(12, vertex_buffer, visibility=ShaderStage.ALL),
            UniformBinding(81, only_count),
        ]
        cut_buffer_size = 4096 * 1000
        # cut_buffer_size = 64 * (count if count is not None else 128)
        print("cut_buffer_size = ", cut_buffer_size)
        cut_trigs_buffer = self.device.createBuffer(
            label="cut trigs " + str(count),
