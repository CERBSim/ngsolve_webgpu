...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
  let subdivision = u_iso.subdivision;
  let n_tets = u_ntets;
  for (var i = id.x; i<n_tets*my_pow(8u,subdivision); i+=256u*1024u) {
    ...
    var elnr = i / my_pow(8u,subdivision);
    var ind = i;
    for(var level = 0u; level < subdivision; level++)
      {
        var sub = ind % 8u;
        ind /= 8u;
    ...
    let lam = array(b0.xyz,
                    ...
                    b2.xyz,
                    b3.xyz);
    let f = array(evalTet(&levelset_values, elnr, 0, b0.xyz) - u_iso.value,
                  evalTet(&levelset_values, elnr, 0, b1.xyz) - u_iso.value,
                  evalTet(&levelset_values, elnr, 0, b2.xyz) - u_iso.value,
                  evalTet(&levelset_values, elnr, 0, b3.xyz) - u_iso.value);
