...
class ContactPairs(ShapeRenderer):
    def __init__(self, mesh, contact, thickness=0.001, color=[0.5, 0.0, 1.0, 1.0], **kwargs):
        self.mesh = mesh
        self.contact = contact
        self.thickness = thickness
        cyl = generate_cylinder(8, thickness, 1.0, top_face=True, bottom_face=False)
    ...
    def update(self, options):
        ...
        self.directions = [
            [data[i + 3 + j] - data[i + j] for j in range(3)] for i in range(0, len(data), 6)
        ]
